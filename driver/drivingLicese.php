...
if(isset($_POST['userId']))
{
    ...
    $check_user_query = "SELECT * FROM driving_licese_info WHERE user_id = '$id'";
    ...
    else
    {
        if((isset($_FILES['frontDL']) && !empty( $_FILES['frontDL']['tmp_name'])) && (isset($_FILES['backDL']) && !empty($_FILES['backDL']['tmp_name'])) && (isset($_FILES['selfiwithDL']) && !empty($_FILES['selfiwithDL']['tmp_name'])))
        {
            ...
            if(!empty($frontDL) && !empty($backDL) && !empty($selfiwithDL))
            {
                ...
                $insert_query = "INSERT INTO driving_licese_info(user_id,driving_licese_no,expiration_date,front_photo_DL,back_photo_DL,selfi_with_DL)VALUES('$id','$DLnumber','$expirationDate','$frontpath','$backpath','$selfipath')";
